...
*** Pages
    ...
    =pva-workbook=  is the main page for the PVA analysis. It shos the user
      ...
      (defmethod event-msg-handler :data-import/csv-sent
        ...
        (when ?data
          (let [uid         (get-in ring-req [:session :uid])
                parsed-data (csv/read-csv (:file-body ?data))
                reply-msg   [:data-import/file-returned parsed-data]]
